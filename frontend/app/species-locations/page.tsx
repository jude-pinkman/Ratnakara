'use client';

import { useState, useEffect } from 'react';
import { speciesLocationsAPI } from '@/lib/api';
import dynamic from 'next/dynamic';
import { Search, MapPin, Fish, Info } from 'lucide-react';

const MapView = dynamic(() => import('@/components/MapView'), { ssr: false });

interface SpeciesLocation {
  latitude: number;
  longitude: number;
  locality: string;
  count: number;
}

interface SpeciesData {
  scientificName: string;
  locations: SpeciesLocation[];
  totalRecords: number;
}

export default function SpeciesLocationsPage() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedSpecies, setSelectedSpecies] = useState<SpeciesData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [mapLocations, setMapLocations] = useState<any[]>([]);
  const [stats, setStats] = useState<any>(null);
  const [suggestions, setSuggestions] = useState<string[]>([]);
  const [showSuggestions, setShowSuggestions] = useState(false);

  // Load stats on mount
  useEffect(() => {
    const loadStats = async () => {
      try {
        const res = await speciesLocationsAPI.getStats();
        if (res.data.success) {
          setStats(res.data.data);
        }
      } catch (err) {
        console.error('Error loading stats:', err);
      }
    };
    loadStats();
  }, []);

  // Update map when species is selected
  useEffect(() => {
    if (selectedSpecies && selectedSpecies.locations) {
      const markers = selectedSpecies.locations.map((loc, idx) => ({
        id: `${selectedSpecies.scientificName}-${idx}`,
        latitude: loc.latitude,
        longitude: loc.longitude,
        title: `${selectedSpecies.scientificName}`,
        description: `${loc.locality} (${loc.count} record${loc.count > 1 ? 's' : ''})`,
        popup: `
          <div style="min-width: 200px;">
            <strong style="color: #1e40af; font-size: 14px;">${selectedSpecies.scientificName}</strong>
            <p style="margin: 8px 0; color: #374151;">${loc.locality}</p>
            <p style="font-size: 12px; color: #6b7280;">📍 ${loc.latitude.toFixed(4)}, ${loc.longitude.toFixed(4)}</p>
            <p style="font-size: 12px; color: #059669; font-weight: 600; margin-top: 4px;">Records: ${loc.count}</p>
          </div>
        `,
        markerColor: '#ef4444',
      }));
      setMapLocations(markers);
    } else {
      setMapLocations([]);
    }
  }, [selectedSpecies]);

  // Auto-suggest species names
  useEffect(() => {
    if (searchQuery.length >= 2 && stats?.topSpecies) {
      const filtered = stats.topSpecies
        .map((sp: any) => sp.scientificName)
        .filter((name: string) => 
          name.toLowerCase().includes(searchQuery.toLowerCase())
        )
        .slice(0, 5);
      setSuggestions(filtered);
    } else {
      setSuggestions([]);
    }
  }, [searchQuery, stats]);

  const handleSearch = async () => {
    if (!searchQuery.trim()) {
      setError('Please enter a species name');
      return;
    }

    setLoading(true);
    setError('');
    setSelectedSpecies(null);

    try {
      const res = await speciesLocationsAPI.search(searchQuery);
      if (res.data.success && res.data.data) {
        setSelectedSpecies(res.data.data);
      } else {
        setError('Species not found in India dataset. Try searching for common marine species names.');
      }
    } catch (err: any) {
      setError(err.message || 'Error searching for species');
    } finally {
      setLoading(false);
    }
  };

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      handleSearch();
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Marine Species Locations</h1>
          <p className="text-gray-600">
            Search for marine species found in India's coastal and marine zones and explore their distribution across locations.
          </p>
        </div>

        {/* Info Banner */}
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-lg shadow-lg p-6 mb-8 text-white">
          <div className="flex items-start gap-4">
            <Info className="w-6 h-6 flex-shrink-0 mt-1" />
            <div>
              <h3 className="font-semibold text-lg mb-2">🇮🇳 Indian Marine Species Database</h3>
              <p className="text-blue-50 text-sm">
                This database contains <strong>{stats?.totalSpecies || '1000+'}</strong> marine species found exclusively in Indian waters including 
                Arabian Sea, Bay of Bengal, Andaman & Nicobar Islands, and Lakshadweep. All coordinates are from Indian coastal and marine zones.
              </p>
            </div>
          </div>
        </div>

        {/* Stats Cards */}
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-blue-600">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-600 mb-2">Total Species</h3>
                  <div className="text-3xl font-bold text-blue-600">{stats.totalSpecies}</div>
                  <p className="text-xs text-gray-500 mt-1">🇮🇳 India-only locations</p>
                </div>
                <Fish className="w-12 h-12 text-blue-200" />
              </div>
            </div>
            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-600">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-600 mb-2">Total Locations</h3>
                  <div className="text-3xl font-bold text-green-600">{stats.totalLocations}</div>
                  <p className="text-xs text-gray-500 mt-1">From taxonomy dataset</p>
                </div>
                <MapPin className="w-12 h-12 text-green-200" />
              </div>
            </div>
            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-600">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-600 mb-2">Avg Locations/Species</h3>
                  <div className="text-3xl font-bold text-purple-600">{stats.averageLocationsPerSpecies}</div>
                  <p className="text-xs text-gray-500 mt-1">per species</p>
                </div>
                <div className="text-4xl">📊</div>
              </div>
            </div>
          </div>
        )}

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Search & Info Panel */}
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow border border-gray-200">
              <div className="border-b border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900">Search Species</h2>
                <p className="text-sm text-gray-500 mt-1">Find species in India's marine zones</p>
              </div>
              <div className="p-6 space-y-4">
                <div className="space-y-2 relative">
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <Search className="w-4 h-4" />
                    Species Name (India Only)
                  </label>
                  <input
                    placeholder="e.g., Homolax megalops, Heterocarpus..."
                    value={searchQuery}
                    onChange={(e) => {
                      setSearchQuery(e.target.value);
                      setShowSuggestions(true);
                    }}
                    onKeyPress={handleKeyPress}
                    onFocus={() => setShowSuggestions(true)}
                    onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
                      {suggestions.map((name, idx) => (
                        <button
                          key={idx}
                          onClick={() => {
                            setSearchQuery(name);
                            setShowSuggestions(false);
                            handleSearch();
                          }}
                          className="w-full text-left px-3 py-2 hover:bg-blue-50 text-sm text-gray-700 border-b last:border-b-0"
                        >
                          <Fish className="w-3 h-3 inline mr-2 text-blue-600" />
                          {name}
                        </button>
                      ))}
                    </div>
                  )}
                </div>
                <button
                  onClick={handleSearch}
                  disabled={loading}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
                  {loading ? 'Searching...' : 'Search'}
                </button>

                {error && (
                  <div className="p-3 bg-red-50 border border-red-200 rounded text-red-700 text-sm">
                    {error}
                  </div>
                )}

                {selectedSpecies && (
                  <div className="p-4 bg-gradient-to-br from-blue-50 to-cyan-50 border-2 border-blue-300 rounded-lg space-y-3 mt-4">
                    <div className="flex items-start gap-3">
                      <Fish className="w-6 h-6 text-blue-600 flex-shrink-0 mt-1" />
                      <div className="flex-1">
                        <h3 className="font-bold text-lg text-gray-900">{selectedSpecies.scientificName}</h3>
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div className="bg-white rounded px-2 py-1">
                            <p className="text-xs text-gray-500">Total Records</p>
                            <p className="text-sm font-semibold text-blue-600">{selectedSpecies.totalRecords}</p>
                          </div>
                          <div className="bg-white rounded px-2 py-1">
                            <p className="text-xs text-gray-500">Locations</p>
                            <p className="text-sm font-semibold text-green-600">{selectedSpecies.locations.length}</p>
                          </div>
                        </div>
                      </div>
                    </div>

                    <div className="border-t border-blue-200 pt-3">
                      <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <MapPin className="w-4 h-4 text-blue-600" />
                        Locations in India ({selectedSpecies.locations.length}):
                      </h4>
                      <div className="space-y-2 max-h-64 overflow-y-auto">
                        {selectedSpecies.locations.map((loc, idx) => (
                          <div
                            key={idx}
                            className="text-xs p-3 bg-white rounded-lg border border-blue-200 hover:border-blue-400 transition shadow-sm"
                          >
                            <p className="font-semibold text-gray-900 mb-1">📍 {loc.locality}</p>
                            <p className="text-gray-600 font-mono text-xs">
                              {loc.latitude.toFixed(4)}°N, {loc.longitude.toFixed(4)}°E
                            </p>
                            <p className="text-green-600 font-semibold mt-1">✓ {loc.count} record{loc.count > 1 ? 's' : ''}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>
                )}

                {/* Top Species */}
                {stats && stats.topSpecies && (
                  <div className="border-t pt-4 mt-4">
                    <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                      <span className="text-lg">🔥</span>
                      Most Recorded Species
                    </h4>
                    <div className="space-y-2">
                      {stats.topSpecies.slice(0, 8).map((sp: any, idx: number) => (
                        <button
                          key={idx}
                          onClick={() => {
                            setSearchQuery(sp.scientificName);
                            setShowSuggestions(false);
                            speciesLocationsAPI.search(sp.scientificName).then((res) => {
                              if (res.data.success && res.data.data) {
                                setSelectedSpecies(res.data.data);
                              }
                            });
                          }}
                          className="w-full text-left p-3 text-xs hover:bg-blue-50 rounded-lg transition border border-gray-200 hover:border-blue-400 hover:shadow-md"
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <p className="font-semibold text-gray-900 mb-1">{sp.scientificName}</p>
                              <p className="text-gray-600">{sp.recordCount} records • {sp.locationCount} locations</p>
                            </div>
                            <Fish className="w-4 h-4 text-blue-400" />
                          </div>
                        </button>
                      ))}
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>

          {/* Map */}
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow border border-gray-200 h-full">
              <div className="border-b border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900">Distribution Map</h2>
                <p className="text-sm text-gray-500 mt-1">
                  {selectedSpecies
                    ? `Showing ${selectedSpecies.locations.length} locations for ${selectedSpecies.scientificName}`
                    : 'Select a species to view its distribution'}
                </p>
              </div>
              <div className="p-6 h-96 md:h-screen max-h-screen">
                {mapLocations.length > 0 ? (
                  <MapView
                    data={mapLocations}
                    type="edna"
                  />
                ) : (
                  <div className="flex items-center justify-center h-full bg-gray-50 rounded text-gray-500">
                    <div className="text-center">
                      <p className="text-lg font-medium mb-2">No Locations</p>
                      <p className="text-sm">Search for a species to view locations on the map</p>
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
